...
public class ElevatorCommands extends Command {
    ...
    public Command RunElevator(DoubleSupplier voltage ,Elevator elevator){
        return Commands.runEnd(()->elevator.getIO().setVolatge(voltage.getAsDouble()) ,()-> elevator.getIO().stopElevator(),elevator).withName("elevator normal voltage");
    }
    ...
    public Command goToPosition(double goal ,Elevator elevator){
        return new FunctionalCommand(
            ...
            ,elevator).withName("go to position:" + goal);
    }
        ...
        elevator.getIO().resetPID(0);},
         ()-> elevator.getIO().setGoal(0),
          interrupted -> {elevator.getIO().stopElevator();},
           ()-> elevator.getIO().atGoal(),
            elevator).withName("elevator down").
            andThen(()->elevator.getIO().setSpeed(-0.1)).
